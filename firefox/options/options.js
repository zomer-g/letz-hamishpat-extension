(function () {
  ...
  const els = {
    ...
    driveConnect: $('drive-connect'), driveAccount: $('drive-account'), driveSkip: $('drive-skip'),
  ...
  els.driveConnect.addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({ type: 'cd/driveConnect' }, (resp) => {
      if (chrome.runtime.lastError) { setStatus(els.driveAccount, 'שגיאה: ' + chrome.runtime.lastError.message, 'err'); return; }
      if (resp && resp.ok) setStatus(els.driveAccount, '✓ מחובר' + (resp.email ? ' (' + resp.email + ')' : ''), 'ok');
      else setStatus(els.driveAccount, 'נכשל: ' + ((resp && resp.error) || 'אין client_id מוגדר? ראה SETUP.md'), 'err');
    });
  });
