...
(function (root) {
  ...
  function open(hearings, meta, settings) {
    ...
    // Anchor: today if it's inside the data span; else the first hearing; else
    // (no hearings at all) the requested range start, so an empty result still
    // shows the period the user asked about instead of the current week.
    let anchor = (minD && maxD && today >= minD && today <= maxD)
      ? today
      : (minD || parseDMY(meta && meta.from) || today);
  ...
  function shellHtml() {
    ...
    // If the harvested text came back with ◆ (U+FFFD) the site's Hebrew was
    // mis-decoded during collection — surface it (with the charset) instead of
    // silently showing gibberish, so the cause is diagnosable.
    const corrupt = (st.hearings || []).some((h) => /�/.test((h.caseName || '') + (h.sittingType || '') + (h.proceeding || '')));
    return '<div class="cd-jcal" role="dialog" aria-label="יומן שופט">' +
      '<div class="cd-jcal__head">' +
        '<div class="cd-jcal__title">📅 יומן השופט' +
          ...
          (corrupt ? ' <span class="cd-jcal__warn" style="background:#b91c1c;color:#fff;border-radius:6px;padding:1px 7px;font-size:12px;" title="הטקסט חזר פגום מהאתר (קידוד)">⚠ קידוד: ' + esc(m.charset || (typeof document !== 'undefined' ? document.characterSet : '?')) + '</span>' : '') +
        '</div>' +
  ...
  function eventLinesHtml(h, compact) {
    ...
    // Case numbers, times and party names mix Hebrew with digits/Latin. In an
    // RTL container that scrambles the character order ("gibberish") unless each
    // such token gets its own bidi-isolation context, so wrap them in <bdi>.
    if (st.fields.time && h.time) parts.push('<span class="cd-jcal__t"><bdi>' + esc(h.time) + '</bdi></span>');
    if (st.fields.sittingType && h.sittingType) parts.push('<span><bdi>' + esc(h.sittingType) + '</bdi></span>');
    const idbits = [];
    if (st.fields.caseType) { const ab = caseTypeAbbr(h); if (ab) idbits.push('<b><bdi>' + esc(ab) + '</bdi></b>'); }
    if (st.fields.caseId && sp.num) idbits.push('<bdi>' + esc(sp.num) + '</bdi>');
    if (idbits.length) parts.push('<span>' + idbits.join(' ') + '</span>');
    if (st.fields.caseName && h.caseName) parts.push('<span class="cd-jcal__nm"><bdi>' + esc(h.caseName) + '</bdi></span>');
    ...
    const head = (st.fields.time && h.time) ? '<div class="cd-jcal__ev-time"><bdi>' + esc(h.time) + '</bdi></div>' : '';
