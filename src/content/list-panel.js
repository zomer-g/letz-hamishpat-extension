...
(function () {
  ...
  const state = { items: [], selected: new Set(), busy: false, cancel: false, els: {}, obs: null, raf: false, settings: null, recipe: null, docFormat: 'pdf', lastClickRowIndex: null, wordSlowIndex: false, nameByTitle: false, defenseMode: false };
  ...
  function initListPanel() {
    ...
    if (w.CD && w.CD.getSettings) {
      w.CD.getSettings().then((s) => { state.settings = s; state.docFormat = (s && s.documentFormat) || 'pdf'; state.wordSlowIndex = !!(s && s.wordSlowIndex); state.nameByTitle = !!(s && s.nameByTitle); state.defenseMode = !!(s && s.defenseMode); renderDestChips(); renderFormatToggle(); });
    }
  ...
  function mountPanel() {
    ...
    panel.innerHTML =
      '<div class="cd-panel__bar">' +
        ...
        '<div class="cd-panel__actions">' +
          '<button type="button" class="cd-panel__btn cd-panel__btn--ghost" data-action="all" title="סימון הכל">סמן הכל</button>' +
    ...
    // Inline (default) or the shared floating window, per settings.ui.floating.
    if (window.CD && window.CD.placePanel) window.CD.placePanel(panel, gridRoot);
    else if (gridRoot && gridRoot.parentElement) gridRoot.parentElement.insertBefore(panel, gridRoot);
  ...
  function bindPanel(panel) {
    ...
    panel.addEventListener('change', (e) => {
      ...
      if (c) { setWordIndex(c.checked); return; }
      const t = e.target.closest('[data-namebytitle]');
      if (t) { setNameByTitle(t.checked); return; }
      const d = e.target.closest('[data-defense]');
      if (d) setDefenseMode(d.checked);
    });
  ...
  function renderFormatToggle() {
    ...
    const isProtocol = !!(adapter.isProtocolScreen && adapter.isProtocolScreen(document));
    const supportsWord = adapter.supportsWordDownload(document);
    const defense = isProtocol && state.defenseMode; // protocols only; PDF-only
    const isDoc = !defense && supportsWord && state.docFormat === 'doc';
    const perDoc = !isDoc || state.wordSlowIndex; // PDF, or "Word with index" → file per doc
    const parts = [];
    if (supportsWord && !defense) {
      // Compact segmented PDF/Word switch (no "פורמט:" prefix — the buttons are
      // self-evident, and dropping it buys width so every control fits one row).
      parts.push('<span class="cd-fmt__seg"><button type="button" class="cd-fmt__b' + (!isDoc ? ' is-active' : '') + '" data-fmt="pdf">PDF</button>' +
        '<button type="button" class="cd-fmt__b' + (isDoc ? ' is-active' : '') + '" data-fmt="doc">Word</button></span>');
      if (isDoc) parts.push('<label class="cd-fmt__idx"><input type="checkbox" data-wordidx' + (state.wordSlowIndex ? ' checked' : '') + '> אינדקס</label>');
    }
    // Per-document naming: name by the document's own name in Net HaMishpat.
    if (perDoc) parts.push('<label class="cd-fmt__idx"><input type="checkbox" data-namebytitle' + (state.nameByTitle ? ' checked' : '') + '> שם מסמך</label>');
    // "סניגוריה" — protocols only: each protocol PDF keeps just first + last page.
    if (isProtocol) parts.push('<label class="cd-fmt__idx"><input type="checkbox" data-defense' + (state.defenseMode ? ' checked' : '') + '> סניגוריה</label>');
    el.innerHTML = parts.length ? '<span class="cd-fmt">' + parts.join('') + '</span>' : '';
  }
  ...
  function setNameByTitle(v) {
    state.nameByTitle = !!v;
    if (w.CD && w.CD.saveSettings) { try { w.CD.saveSettings({ nameByTitle: state.nameByTitle }); } catch (e) {} }
    setStatus(v
      ? 'שם הקובץ יהיה לפי שם המסמך בנט המשפט (למשל "כתב תביעה").'
      : 'שם הקובץ יהיה לפי סוג + תאריך + מספר תיק.', 'idle');
  }
  function setDefenseMode(v) {
    state.defenseMode = !!v;
    const patch = { defenseMode: state.defenseMode };
    if (v) { state.docFormat = 'pdf'; patch.documentFormat = 'pdf'; } // defense → PDF only
    if (w.CD && w.CD.saveSettings) { try { w.CD.saveSettings(patch); } catch (e) {} }
    renderFormatToggle();
    setStatus(v
      ? 'מצב סניגוריה: מכל פרוטוקול יורד רק העמוד הראשון והאחרון, כ-PDF.'
      : 'מצב סניגוריה כובה — חוזר להורדה מלאה.', 'idle');
  }
  ...
  function syncRows() {
    ...
    for (const r of rows) {
      ...
      if (!cb) {
        ...
        // Non-protocol screens (decisions / motions / paper file): an ABSOLUTE
        // overlay box on the cell's RTL start (right) edge — like the hearings
        // grid — so the selector is consistently on the right. It MUST be out of
        // flow: an inline box inside the name cell's flex layout squeezed the name
        // to zero width and it vanished. Protocols keep the plain inline box in
        // their roomy icon cell (native checkbox hidden).
        cb.type = 'checkbox';
        cb.className = r.hideNative ? 'cd-row-cb' : 'cd-row-cb cd-row-cb--overlay cd-row-cb--right';
        if (!r.hideNative) {
          // Anchor the absolute box to the cell WITHOUT overriding its own
          // position (AG-Grid cells are already position:absolute; forcing
          // relative collapsed columns) — only add it on a genuinely static cell.
          if (getComputedStyle(cell).position === 'static') cell.style.position = 'relative';
          // Open a gutter on the right (RTL start) so the right-aligned text
          // clears the box. border-box keeps the gutter inside the column width
          // (no widening → no column-collapse); the column is wide enough that
          // shifting the text left doesn't truncate it.
          cell.style.boxSizing = 'border-box';
          cell.style.paddingInlineStart = '18px';
        }
        cb.title = 'לבחירה להורדה (Shift = טווח)';
        cb.addEventListener('click', (ev) => {
          ev.stopPropagation();
          // Resolve the doc id FRESH from the checkbox's dataset (re-bound on
          // every syncRows pass) so a sorted grid or a transient empty-cell
          // render can't leave this box pointing at the wrong document.
          const docId = cb.dataset.cdDocId;
          if (!docId) return;
          const ri = parseInt(cb.dataset.cdRowIndex, 10);
          // Shift-click: (de)select the contiguous range in DISPLAY order
          // (by row-index), so the visual range matches what the user sees even
          // when the grid is sorted differently from the underlying store.
          if (ev.shiftKey && state.lastClickRowIndex != null && !isNaN(ri)) {
            const lo = Math.min(state.lastClickRowIndex, ri), hi = Math.max(state.lastClickRowIndex, ri);
            const want = cb.checked;
            document.querySelectorAll('.cd-row-cb').forEach((other) => {
              const ori = parseInt(other.dataset.cdRowIndex, 10);
              const oid = other.dataset.cdDocId;
              if (!oid || isNaN(ori) || ori < lo || ori > hi) return;
              if (want) state.selected.add(oid); else state.selected.delete(oid);
            });
            syncRows();
          } else {
            if (cb.checked) state.selected.add(docId); else state.selected.delete(docId);
          }
          if (!isNaN(ri)) state.lastClickRowIndex = ri;
          refreshCount();
        });
        cell.insertBefore(cb, cell.firstChild);
      }
      // (Re)bind this checkbox to the row's CURRENT item & display position on
      // every pass — the row→item mapping changes as the grid finishes rendering
      // or is re-sorted, and the click handler reads these dataset values live.
      cb.dataset.cdDocId = String(r.item.docId);
      if (r.rowIndex != null && !isNaN(r.rowIndex)) cb.dataset.cdRowIndex = String(r.rowIndex);
  ...
  // Administrative confirmations ("אישור על הגשת מסמך", "אישור פתיחה לתיק/לבקשה")
  // are auto-generated receipts, not substantive documents. We skip them in
  // "סמן הכל" by default; the user can still tick their row to include them.
  const CONFIRMATION_RE = /^\s*אישור/;
  function isConfirmation(it) {
    return CONFIRMATION_RE.test((it && it.docKind) || '');
  }
  ...
  function selectAll() {
    ...
    for (const it of state.items) {
      if (isConfirmation(it)) { skipped++; continue; }
      state.selected.add(it.docId);
    }
    syncRows();
    setStatus('נבחרו ' + state.selected.size + ' מסמכים' +
      (skipped ? ' · דולגו ' + skipped + ' אישורים (סמן ידנית אם דרוש)' : ''));
  }
  ...
  async function startDownload() {
    ...
    // Defense mode (protocols only) forces the per-page PDF path — never Word —
    // so a stale stored documentFormat='doc' can't bypass the first+last slice.
    const defenseActive = state.defenseMode && !!(adapter.isProtocolScreen && adapter.isProtocolScreen(document));
    if (!defenseActive && state.docFormat === 'doc' && adapter.supportsWordDownload(document)) {
      state.items = adapter.listAllItems(document);
    ...
    async function processOne(it) {
      ...
      // "סניגוריה" mode (protocols screen only): keep just the first + last page.
      const defense = state.defenseMode && !!(adapter.isProtocolScreen && adapter.isProtocolScreen(document));
      if (defense && images.length > 2) images = [images[0], images[images.length - 1]];
      const ord = already + ok + 1;
      let bytes;
      try { bytes = await buildPdfBytes(images, ord, totalChosen); } catch (e) { return false; }
      ok++;
      const filename = pdfName(ord, it, caseId, defense ? '(עמ׳ ראשון ואחרון)' : '');
    ...
    if (dests.drive) {
      if (!driveReady) summary.push('Drive ✗ (' + (driveSetupErr || 'לא מחובר — ⚙').slice(0, 30) + ')');
        ...
        const dfn = (state.settings && state.settings.drive && state.settings.drive.folderName) || 'מסמכי נט המשפט';
  ...
  // Informative base name, DATE FIRST so the files sort chronologically by name:
  // "<YYYY-MM-DD> <label> <caseId>" (e.g. "2025-02-19 החלטה 56708-12-22").
  // <label> is the document's Net HaMishpat name when "name by title" is on,
  // otherwise its type. The date is always prefixed (when known) — including in
  // the name-by-title case, which previously dropped it.
  function infoStem(it, caseId) {
    const ymd = toYmd(it && it.date);
    let label;
    if (state.settings && state.settings.nameByTitle && it && it.title && String(it.title).trim()) {
      label = String(it.title).trim();
    } else {
      label = docTypeWord(it.docType);
    }
    return [ymd, label, caseId].filter(Boolean).join(' ');
  }
  ...
  function pdfName(ord, it, caseId, suffix) {
    let stem = infoStem(it, caseId);
    if (suffix) stem += ' ' + suffix;
    return uniqueName(sanitize(stem, 110), '.pdf');
  }
  function toCsv(rows) {
