...
(function () {
  ...
  const state = {
    ...
    bgToken: null,          // cancellation token of an in-flight BACKGROUND judge collection
  };
  ...
  function mountPanel() {
    ...
    const floating = !!(w.CD && w.CD.uiFloating && w.CD.uiFloating());
    if (w.CD && w.CD.placePanel) {
      w.CD.placePanel(panel, gridRoot);
    } else if (gridRoot && gridRoot.parentElement) {
      gridRoot.parentElement.insertBefore(panel, gridRoot);
    } else {
      document.body.insertBefore(panel, document.body.firstChild);
    }
    // Inline only: align the panel to the grid below it (width + horizontal
    // offset). In the floating window it stretches to the window's width.
    if (!floating && gridRoot && gridRoot.parentElement && panel.parentElement === gridRoot.parentElement) {
  ...
  function openJudgeCalendar() {
    ...
    if (state.bgToken) { setStatus('האיסוף כבר רץ ברקע — יש להמתין לסיומו.'); return; }
  ...
  function showJudgeCalendar() {
    ...
    // Open even when empty — the user asked to see the calendar; an empty one
    // for the chosen range is clearer than nothing.
    w.CD.openJudgeCalendar(hearings, state.judgeMeta || {}, state.settings || {});
    if (!hearings.length) setStatus('לא נמצאו דיונים לשופט/ת בטווח שנבחר — היומן מוצג ריק.');
  }
  ...
  async function judgeAction(action) {
    ...
    // calview/download: collect in a NEW TAB via the judge-runner state machine —
    // the ONE collection engine (single actor, live-page postbacks, rendered-day
    // verification, WAF pacing, date stamping). Collecting inside THIS tab was
    // unfixably fragile: multiple extension actors share the page (watchdogs,
    // header-sync jobs), a post-finish reload killed the calendar, and days came
    // back duplicated/lost. 'sync' still runs the in-tab flow (it needs this
    // tab's Google-calendar context).
    if (action === 'calview' || action === 'download') {
      judgeOpenRunnerTab();
      return;
    }
  ...
  // Launch the judge-runner in a new tab for the court+judge selected on THIS
  // report page and the panel's date range. Mirrors case-judge-chip.runRequest.
  function judgeOpenRunnerTab() {
    const pre = judgeValidate();
    if (!pre) return;
    const judgeName = (pre.ctx.judgeName || '').trim();
    const courtName = (pre.ctx.courtName || '').trim();
    if (!judgeName || !courtName || /בחר/.test(judgeName + courtName)) {
      setStatus('יש לבחור בית משפט ושופט/ת בעמוד נט המשפט (למעלה) תחילה.', 'error');
      return;
    }
    const id = 'j' + Date.now().toString(36) + '_' + Math.floor(Math.random() * 1e6).toString(36);
    const req = { judge: judgeName, court: courtName, from: pre.from, to: pre.to, action: 'calview' };
    ...
    try {
      chrome.storage.local.set({ ['cd_jreq_' + id]: req }, () => {
        ...
        // Land on the right portal for THIS domain (the runner drives the menu
        // from there): public www → NGCS.Web.Site; secured → Ngcs.Web.Secured.
        ...
        chrome.runtime.sendMessage({ type: 'cd/openTab', url: landing }, (r) => {
          ...
          setStatus('✓ נפתח טאב חדש — האיסוף וההצגה רצים שם. אפשר להמשיך לעבוד כאן.');
        });
      });
    } catch (e) {
  ...
  function syncRowCheckboxes() {
    ...
    for (const rowEl of rowEls) {
      ...
      if (!cb) {
        ...
        // cd-row-cb--overlay: positioned absolutely over the cell's free
        // (inline-end) edge so it doesn't push the date text and truncate it.
        // The first cell here is the date column, unlike the protocols screen
        // where the box goes into a roomy icon cell.
        // --right: place the box on the RTL start (right) of the date cell.
        cb.type = 'checkbox'; cb.className = 'cd-row-cb cd-row-cb--overlay cd-row-cb--right'; cb.title = 'לבחירה להורדה';
        ...
        // Anchor the absolute checkbox WITHOUT disturbing the grid's own layout:
        // AG-Grid cells are already position:absolute (a valid containing block),
        // and overriding that to `relative` breaks the grid's column-width
        // calculation — it collapsed the date column to 0 width. Only add
        // positioning when the cell is genuinely static.
        if (getComputedStyle(cell).position === 'static') cell.style.position = 'relative';
        // Left-align the date so its text clears the right-side box (the column is
        // only just wide enough for the date — right-alignment would let the box
        // clip the year's last digit).
  ...
  // Shared validation for both collection mechanisms. Returns
  // {days, from, to, ctx} or null (after setting an error status).
  function judgeValidate() {
    if (readJob()) { setStatus('איסוף כבר רץ — יש להמתין לסיומו (או לבטל בחלון האיסוף).', 'error'); return null; }
    if (state.bgToken) { setStatus('איסוף כבר רץ ברקע — יש להמתין לסיומו (או לבטל בחלון האיסוף).', 'error'); return null; }
    const from = state.els.from && state.els.from.value.trim();
    const to = state.els.to && state.els.to.value.trim();
    if (!from || !to) { setStatus('יש לבחור תאריך התחלה וסיום (לחיצה על שדה התאריך פותחת לוח שנה).', 'error'); return null; }
    const days = enumerateDays(from, to);
    if (!days) { setStatus('טווח תאריכים לא תקין (ההתחלה חייבת להיות לפני הסיום).', 'error'); return null; }
    if (days.length > MAX_RANGE_DAYS) { setStatus('הטווח גדול מ-' + MAX_RANGE_DAYS + ' ימים. יש לבחור טווח קצר יותר.', 'error'); return null; }
    const ctx = adapter.judgeContext(document);
    if (!ctx.judgeValue && !ctx.judgeName) {
      setStatus('יש לבחור שופט/ת (גורם שיפוטי) בעמוד לפני האיסוף.', 'error'); return null;
    }
    return { days: days, from: from, to: to, ctx: ctx };
  }
  ...
  //
  // Mechanism: prefer the BACKGROUND collector (fetch-replayed postbacks — the
  // page never reloads; the user sees only the progress card). If it can't
  // verify data on this deployment (or errors), fall back automatically to
  // the legacy visible flow (per-day form.submit + sessionStorage resume).
  function judgeStart(opts) {
    opts = opts || {};
    const pre = judgeValidate();
    if (!pre) return false;
    // ALWAYS use the visible (navigation) flow. The background fetch collector
    // posts from INERT DOMParser documents (the site's inline scripts never
    // ran in them), and on Net HaMishpat every SECOND such post fails to raise
    // the report event — the server then re-renders the PREVIOUS report, which
    // got recorded under the next day (the deterministic duplicate/empty-days
    // bug: 13 empty, 15=14, 17=16). The visible flow posts from LIVE pages —
    // verified correct for every day — and now carries rendered-day
    // verification + WAF pacing. (CD.judgeBg is kept for tests/tooling only.)
    return judgeStartLegacy(opts, pre);
  }

  // Background (no-reload) collection.
  async function judgeStartBg(opts, pre) {
    const token = { cancelled: false };
    state.bgToken = token;
    const n = pre.days.length;
    const startMsg = 'אוסף ברקע: ' + n + ' ימים עבור ' + (pre.ctx.judgeName || 'השופט/ת שנבחרו') + '…';
    setStatus(startMsg);
    showCollectMask(startMsg, 0);
    let res;
    try {
      res = await w.CD.judgeBg.collectRange({
        seedDoc: document, seedUrl: location.href, days: pre.days, token: token,
        onProgress: function (done, total, count) {
          const msg = done >= total
            ? 'מסכם: נאספו ' + count + ' דיונים…'
            : 'אוסף ברקע יום ' + (done + 1) + '/' + total + ' (' + pre.days[done] + ') · נאספו ' + count + ' דיונים…';
          setStatus(msg);
          showCollectMask(msg, done / total);
        },
      });
    } catch (e) {
    ...
    if (token.cancelled || (res && res.cancelled)) {
      hideCollectMask();
      setStatus('האיסוף בוטל.');
      return;
    }
    if (res && res.ok) {
      judgeFinish({
        rows: res.hearings, days: pre.days, from: pre.from, to: pre.to,
        courtName: pre.ctx.courtName, judgeName: pre.ctx.judgeName,
        thenAction: opts.thenAction || (opts.openCalWhenDone ? 'calview' : null),
        active: false,
      }, '');
      return;
    }
    // Background mechanism unavailable here → the visible flow still works.
    hideCollectMask();
    setStatus('איסוף ברקע לא זמין בעמוד זה (' + ((res && res.error) || 'שגיאה') + ') — עובר לאיסוף רגיל…');
    judgeStartLegacy(opts, pre);
  }

  // Legacy visible flow: one full ASP.NET postback per day, resumed across
  // the page reloads via sessionStorage (see JUDGE_JOB_KEY block comment).
  function judgeStartLegacy(opts, pre) {
    opts = opts || {};
    const days = pre.days, from = pre.from, to = pre.to, ctx = pre.ctx;
    const job = {
      active: true,
      days: days,
  ...
  async function judgeOnMount() {
    ...
    // ── Rendered-day verification ─────────────────────────────────────────
    // The site's WAF intermittently BLOCKS rapid successive postbacks (a
    // "חסימת בקשה לא מורשת" page, or a bounce that re-renders the previous
    // report). Harvesting such a page blindly recorded the PREVIOUS day's rows
    // under this day (the duplicate-days bug) or a false empty. A page is only
    // OUR day's report if its date field round-tripped our value — verify, and
    // when it doesn't match, re-submit this day (paced) instead of harvesting.
    const dEl = adapter.findSingleDateInput(document);
    if (!dEl || (dEl.value || '').trim() !== day) {
      job.retries = job.retries || {};
      const tries = job.retries[day] || 0;
      if (tries < 3) {
        job.retries[day] = tries + 1;
        writeJob(job); // collected unchanged → next load re-verifies this day
        setStatus('העמוד אינו מציג את ' + day + ' — מגיש שוב (' + (tries + 1) + '/3)…');
        await sleep(1800 * (tries + 1)); // back off — the WAF is rate-based
        if (adapter.submitJudgeReport(document, day)) return;
      }
      job.active = false; writeJob(job);
      judgeFinish(job, 'נעצר ביום ' + day + ' — האתר חסם את רצף הבקשות. נסה/י שוב עם טווח קצר יותר.');
      return;
    }

    const rows = await harvestCurrentDay();
    // Mojibake retry (see judge-runner.js): the server sometimes returns this
    // day's response UTF-8-bodied while the tab decodes windows-1255 (Hebrew →
    // ◆). Per-response quirk → re-submit the SAME day via the normal postback
    // (up to 2 retries). Never fetch-replay — it desyncs the server's report
    // state and shifts/duplicates days.
    if (CD.judgeHasFFFD && CD.judgeHasFFFD(rows)) {
      job.retries = job.retries || {};
      const tries = job.retries[day] || 0;
      if (tries < 2) {
        job.retries[day] = tries + 1;
        writeJob(job); // collected unchanged → the reload re-harvests this day
        setStatus('קידוד פגום ביום ' + day + ' — מנסה שוב (' + (tries + 1) + '/2)…');
        if (adapter.submitJudgeReport(document, day)) return;
      }
    }
  ...
  async function harvestCurrentDay() {
    // Prefer the embedded ArrayStore: it is server-rendered (present the moment
    // the page loads), holds EVERY row regardless of grid paging, and doesn't
    // depend on AG-Grid finishing its async render — a slow grid used to hit
    // waitGridReady's timeout and record a busy day as EMPTY.
    try {
      const fromStore = adapter.listHearingsFromStore && adapter.listHearingsFromStore(document);
      if (fromStore && fromStore.length) return fromStore;
      if (adapter.hasEmptyHearingStore && adapter.hasEmptyHearingStore(document)) return [];
    } catch (e) {}
    await waitGridReady();
  ...
  function judgeFinish(job, note) {
    ...
    // range is collected — unless the run was cancelled. 'calview' runs even with
    // zero hearings (an empty calendar for the range beats nothing happening);
    // download/sync self-guard on empty inside runJudgeAction.
    if (job.thenAction && !job._cancelled) runJudgeAction(job.thenAction);
  }

  function judgeCancel() {
    // Background run: flip its token; judgeStartBg cleans up when the loop
    // notices (between day fetches).
    if (state.bgToken) { state.bgToken.cancelled = true; return; }
    const job = readJob();
  ...
  function sendToSW(message) {
    ...
    const STALE = 'התוסף עודכן — יש לרענן את העמוד (F5) ולנסות שוב.';
    return new Promise((resolve) => {
      // After the extension is reloaded/updated, an old content script left on
      // the page loses its port: chrome.runtime.id becomes undefined and any
      // sendMessage throws "Cannot read properties of undefined". Surface a
      // clear instruction instead of the raw JS error.
      if (!(typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.id)) {
        resolve({ ok: false, error: STALE, stale: true });
        return;
      }
      try {
        ...
        resolve({ ok: false, error: /context invalidated|sendMessage/i.test(raw) ? STALE : raw });
      }
    });
  }
